...
fn string_literals() {
    ...
    assert!(string_slice == "Hello World".to_string());
}
...
fn string_to_slice() {
    ...
    assert!(slice == "Can't stop me now".to_string());
}
...
fn slicing_the_string() {
    let string = "Boom";
    assert!(&string[0..2] == "Bo".to_string());
}
...
fn slicing_the_byte() {
    let dog = "忠犬ハチ公";
    let slice = &dog[0..2].to_string();
    assert!(slice == &"忠犬".to_string());
}
